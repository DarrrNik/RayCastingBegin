...
void Raycasting::Raycast()
{
	...
	while (true) {
		...
		if (GetAsyncKeyState('A') & 0x8000)
			player.viewAngle -= 1.0f * player.FOV * elapsedTime.count();
		if (GetAsyncKeyState('D') & 0x8000)
			player.viewAngle += 1.0f * player.FOV * elapsedTime.count();
		if (GetAsyncKeyState('W') & 0x8000) {
			if (player.x + sinf(player.viewAngle) * 5.0f * elapsedTime.count() > 1 && player.x + sinf(player.viewAngle) * 5.0f * elapsedTime.count() < MapController::nMapWidth - 1) player.x += sinf(player.viewAngle) * 5.0f * elapsedTime.count();
			if (player.y + cosf(player.viewAngle) * 5.0f * elapsedTime.count() > 1 && player.y + cosf(player.viewAngle) * 5.0f * elapsedTime.count() < MapController::nMapHeight - 1) player.y += cosf(player.viewAngle) * 5.0f * elapsedTime.count();
		}
